...
class SensorValues(object):
    static_sensors = {}
    sensor_functions = {
        'manufacturer': lambda: get_board_info('board_vendor'),
        'model': lambda: get_board_info('board_name'),
        'arch': lambda: quick_command('lscpu', term='Architecture:'),
        'processor': lambda: quick_command('lscpu', term='Model name:'),
        'threads': lambda: quick_command('lscpu', term='CPU(s):', ret_type=int),
        'cores': lambda: quick_command('lscpu', term='Core(s) per socket:', ret_type=int),
        'max_clock': lambda: quick_command('lscpu', term='CPU max MHz:', ret_type=int) / 1000,
        'distro': lambda: quick_cat('/etc/os-release', term='PRETTY_NAME=').strip('"'),
        'boot': lambda: as_local(utc_from_ts(psutil.boot_time())).isoformat(),
        'clock': lambda: quick_command('lscpu', term='CPU MHz:', ret_type=int) / 1000,
        'temp': lambda: get_temp(),
        'usage': lambda: psutil.cpu_percent(interval=None),
        'load_1m': lambda: psutil.getloadavg()[0],
        'load_5m': lambda: psutil.getloadavg()[1],
        'load_15m': lambda: psutil.getloadavg()[2],
        'memory': lambda: psutil.virtual_memory().percent,
        'swap': lambda: psutil.swap_memory().percent,
        'hostname': lambda: socket.gethostname(),
        'updates': lambda: get_updates(),
        'ip': lambda: get_host_ip(),
        'tx': lambda: tx_up.update(),
        'rx': lambda: tx_down.update(),
        'wifi': lambda: ' '.join(quick_cat('/proc/net/wireless', term='wlan0:').split(' ')).split()[2],
        'ssid': lambda: quick_command('/usr/sbin/iwgetid', args=['-r']),
        'message': lambda: str(as_local(utc_from_ts(time.time())).isoformat()),
        'filesystem': lambda: psutil.disk_usage('/').percent}
